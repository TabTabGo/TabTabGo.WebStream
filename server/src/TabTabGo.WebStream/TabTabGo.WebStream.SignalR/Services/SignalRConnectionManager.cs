...
namespace TabTabGo.WebStream.SignalR.Services
{
    public class SignalRConnectionManager : IConnectionManager
    {
        ...
        {
            _hubContext.Clients.Client(connectionId).SendAsync(Constants.HandShakeMessage, $"You connected with connection ID: {connectionId}");
        }

        public async Task RegisterConnectionAsync(string connectionId, string userId, CancellationToken cancellationToken = default)
        {
            await _hubContext.Clients.Client(connectionId).SendAsync(Constants.HandShakeMessage, $"You connected with connection ID: {connectionId}");
        }

        public void ReRegisterConnection(string connectionId, string userId)
        {
            _hubContext.Clients.Client(connectionId).SendAsync(Constants.HandShakeMessage, $"You reconnected with connection ID: {connectionId}");
        }
        ...
        {
            await _hubContext.Clients.Client(connectionId).SendAsync(Constants.HandShakeMessage, $"You reconnected with connection ID: {connectionId}");
        }

        public void UnRegisterConnection(string connectionId, string userId)
        {
            _hubContext.Clients.Client(connectionId).SendAsync(Constants.HandShakeMessage, $"You disconnected with connection ID: {connectionId}");
        }
        ...
        {
           await _hubContext.Clients.Client(connectionId).SendAsync(Constants.HandShakeMessage, $"You disconnected with connection ID: {connectionId}");
        }
    }
}
