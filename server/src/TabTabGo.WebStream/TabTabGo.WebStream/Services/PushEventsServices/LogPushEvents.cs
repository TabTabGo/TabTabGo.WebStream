...
{
    internal class LogPushEvents : IPushEvent
    {
        ILogger<IPushEvent> _logger;
        public LogPushEvents(ILogger<IPushEvent> logger)
        {
            _logger = logger;
        }
        ...
        {
            _logger.LogInformation("pushing message to devices of connections  {@connectionIds} {@message}", connectionIds, message);
            return Task.CompletedTask;
        }

        public Task PushAsync(string connectionId, WebStreamMessage message, CancellationToken cancellationToken = default)
        {
            _logger.LogInformation("pushing message to device of connection {@connectionId} {@message}", connectionId, message);
            return Task.CompletedTask;
        }
        ...
        {
            _logger.LogInformation("pushing message to users of ids {@userIds} {@message}", userIds, message);
            return Task.CompletedTask;
        }

        public Task PushToUserAsync(string userId, WebStreamMessage message, CancellationToken cancellationToken = default)
        {
            _logger.LogInformation("pushing message to user of Id {@userId} {@message}", userId, message);
            return Task.CompletedTask;
        }
    }
}
